...
public class InviteHandler extends DialogMethodHandler
    ...
    private boolean isEarlyMediaStarted = false;
...
//        }
        ...
        if (isFirstProvRespWithToTag) {
            ...
            if (!isEarlyMediaStarted) {
                logger.info("Starting early media with " + sipResponse);

                SessionDescription sessionDescription = sdpManager.parse(sipResponse.getBody());

                if (sessionDescription == null) {
                    logger.info("Session processing failed!");
                    return;
                }
                ...
                try {
                    ...
                    logger.debug("Early media will be reachable at " + mediaDestination.getDestination() + ":" + mediaDestination.getPort() + " with " + mediaDestination.getCodec());
                } catch (NoCodecException e) {
                    e.printStackTrace();
                }
                ...
                isEarlyMediaStarted = true;
            }
            else {
                logger.debug("Early media already started! This provisional response will be ignored!");
            }
        }
